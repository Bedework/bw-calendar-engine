...
//    if (lpos2 > 0) {
      ...
      */
      ...
      for (int redirects = 0; redirects < 10; redirects++) {
        ...
        if (rcode != HttpServletResponse.SC_OK) {
          ...
          throw new BedeworkException("Got response " + rcode +
                                       ", host " + hi.getHostname() +
                                       " and url " + uri);
        }
      ...
      throw new BedeworkException("Too many redirects on " + uri);
  ...
  private void parseResponse(final HostInfo hi,
    ...
    try {
      ...
      if (doc == null){
        throw new BedeworkException(CalFacadeErrorCode.badResponse);
      }
  ...
  public void send(final IscheduleOut iout,
    ...
    try {
      ...
      for (int failures = 0; failures < 10; failures++) {
        ...
        if (req == null) {
          throw new BedeworkException("No method " + iout.getMethod());
        }
...
//          hi.setIScheduleUrl(null);
  ...
  private IscheduleOut makeFreeBusyRequest(final HostInfo hi,
    ...
    //  throw new BedeworkException(CalFacadeErrorCode.schedulingBadRecipients);
    //}
